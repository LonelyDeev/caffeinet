...
(function () {
    ...
    async function load(page) {
        ...
        const res = await App.ajax('/organization/withdrawals/data?page=' + page + '&status=' + currentStatus);
    ...
    function renderPagination(data) {
        if (data.last_page <= 1) {
            pagination.innerHTML = `<span>${data.total.toLocaleString('fa-IR')} درخواست</span>`;
            return;
        }
        pagination.innerHTML = `
            <span>${data.total.toLocaleString('fa-IR')} درخواست — صفحه ${data.current_page.toLocaleString('fa-IR')} از ${data.last_page.toLocaleString('fa-IR')}</span>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await App.ajax('/organization/withdrawals', {
                method: 'POST',
