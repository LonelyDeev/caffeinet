/**
 * کافی‌نت آنلاین — اسکریپت صفحه «تیکت‌های پشتیبانی» (کافی‌نت)
 * فایل مستقل (Blade + jQuery) — بدون Node / بدون بیلد
 */
(function () {
    const NET = window.location.pathname.match(/\/coffeenet\/(\d+)/);
    const netId = NET ? NET[1] : '';
    const base = '/coffeenet/' + netId;
    ...
    async function load(page) {
        ...
        const res = await App.ajax(base + '/tickets/data?' + params.toString());
    ...
    function render(data) {
        ...
        rows.innerHTML = data.data.map(r => `
            <tr>
                <td>
                    <a href="${App.url(base + '/tickets/' + r.id)}" class="block group">
                ...
                <td class="text-center">
                    <a class="btn-ghost !py-2 !px-4 !text-xs ui-press" href="${App.url(base + '/tickets/' + r.id)}">
